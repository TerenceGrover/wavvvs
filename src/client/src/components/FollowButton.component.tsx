...
export default function FollowButton() {
  ...
  React.useEffect(() => {
    setSelf(selectedUser.id === currentUser._id || selectedUser.id === undefined);
    ...
    <div>
      {!self && (
        <button
          ...
          className={`${
            followed ? 'bg-neutral-300' : 'bg-neutral-800'
          } border-neutral-700 border-2  p-2 px-8 rounded-xl`}
        >
          {followed ? (
